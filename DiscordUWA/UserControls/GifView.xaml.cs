...
// todo: this control may or may not be needed as of win10 1607, i must have made this before that build
    ...
    public sealed partial class GifView : UserControl {
        ...
        private async Task UpdateSourceAsync() {
            ...
            if (UriSource != null) {
                ...
                try {
                    var streamReference = RandomAccessStreamReference.CreateFromUri(new Uri(uriSource));
                    var inMemoryStream = new InMemoryRandomAccessStream();

                    using (inMemoryStream) {
                        var readStream = await streamReference.OpenReadAsync().AsTask(cancellationTokenSource.Token);

                        string imageType = readStream.ContentType.ToLowerInvariant();
                        // todo, just go down the list of decoders?
                        if ( imageType != "image/gif" 
                            && imageType != "image/jpeg" && imageType != "image/pjpeg" // the fuck is pjpeg?
                            && imageType != "image/png" && imageType != "image/x-png") {
                        ...
                        if (uriSource.Equals(UriSource)) {
                            //if (imageType == "image/gif") {
                                ...
                                GifImage.Source = await gifPresenter.InitializeAsync(inMemoryStream, imageType);
        ...
        private class GifPresenter {
            ...
            public async Task<ImageSource> InitializeAsync(IRandomAccessStream streamSource, string contentType) {
                ...
                switch (contentType) {
                    case "image/gif":
                        ...
                        break;
                    case "image/pjpeg": // hope the decoder will take a 'pjpeg' as well, bleh
                    case "image/jpeg":
                ...
                _bitmapDecoder = await BitmapDecoder.CreateAsync(decoderId, streamSource);
                if (contentType == "image/gif") {
                    _imageProperties = await RetrieveImagePropertiesAsync(_bitmapDecoder);
                    for (var i = 0u; i < _bitmapDecoder.FrameCount; i++) {
                        var bitmapFrame = await _bitmapDecoder.GetFrameAsync(i);
                        _frameProperties.Add(await RetrieveFramePropertiesAsync(bitmapFrame));
                    }
                }
                else {
                    _imageProperties = new ImageProperties((int)_bitmapDecoder.PixelWidth, (int)_bitmapDecoder.PixelHeight, false, 0);
                    _frameProperties.Add(new FrameProperties(new Rect(0, 0, _bitmapDecoder.PixelWidth, _bitmapDecoder.PixelHeight), 0, false));
                }
